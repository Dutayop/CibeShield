...
echo '=== CIBE SHIELD XDP CORE INSTALL ==='
...
apt install -y linux-tools-$(uname -r) || apt install -y linux-tools-generic
...
#define MAX_PPS 3000
#define MAX_BYTES 800
#define WINDOW 1000000000ULL

struct key { __u32 ip; };
struct val { __u64 ts; __u32 pps; };
...
int xdp_samp(struct xdp_md *ctx) {
  ...
  __u8 *p = (void *)(udp + 1);
  if ((void *)(p + 4) > end) return XDP_PASS;

  if (p[0]!=S0 || p[1]!=S1 || p[2]!=S2 || p[3]!=S3) return XDP_PASS;

  struct key k = { .ip = ip->saddr };
  struct val *v = bpf_map_lookup_elem(&ip_rate, &k);
  __u64 now = bpf_ktime_get_ns();

  if (!v) {
    struct val nv = { .ts = now, .pps = 1 };
  ...
  v->pps++;
  if (v->pps > MAX_PPS) return XDP_DROP;
  return XDP_PASS;
}
...
# COMPILE
# =========================
clang -O2 -target bpf -c samp_xdp.c -o samp_xdp.o
...
systemctl enable --now cibe-xdp

echo '=== CIBE SHIELD XDP ACTIVE ==='
echo "Interface : $IFACE"
echo "Check     : ip link show $IFACE | grep xdp"
EOF
)"
